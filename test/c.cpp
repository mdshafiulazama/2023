            ...
            while (getline(purchaseFile, line))
            {
                if (line.find("Bill No.: ") != string::npos)
                {
                    try {
                        ...
                        // Handle the case where stoi conversion fails (e.g., print an error message)
                        cout << "Error: Invalid bill number format." << endl;
                    }
                }
            }
...
void vat::outputs(int billNumber, int customerID)
{
    ...
    if (customerID > 0) // Only prompt for cash input if a valid customer ID is provided
    {
        int inputID;
        cout << "\n\nEnter customer ID: ";
        cin >> inputID;
...
int main()
{
    ...
    ifstream fin;
    int customerID = 0;
    int billNum = 0; // Declare billNum outside the switch block
    ...
    last:;
        ...
        start:
            ...
            case '1':
                obj.vatcal();
                billNum = generateBillNumber(); // Initialize billNum here
                if (customerID == 0)
                {
                    cout << "No customer ID available. Creating a new customer ID." << endl;
                ...
                obj.outputs(billNum, customerID);
                goto start;
            case '2':
