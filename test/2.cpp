...
int generateCustomerID(vector<Customer> &customers)
{
    ...
    {
        // Find the highest existing customer ID and increment it by 1
...
void vat::outputs(int billNumber, int customerID)
{
    ...
    {
        cout << "Invalid customer ID. Purchase not recorded." << endl;
        return;
    }
...
// Function to display purchase history for a specific customer ID
void displayPurchaseDetails(const vector<Purchase> &purchases, int customerID)
{
    ...
    for (const Purchase &purchase : purchases)
    {
        ...
        {
            cout << "Bill Number: " << purchase.billNo << endl;
